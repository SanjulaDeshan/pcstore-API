...
export default function HeroCarousel({ ads }: { ads: any[] }) {
  ...
  if (!ads || ads.length === 0) {
    ...
    return (
      <section className="relative h-[600px] rounded-[3rem] overflow-hidden border border-white/10 flex items-center group shadow-2xl">
        ...
        <div className="relative z-20 px-10 md:px-20 space-y-8 max-w-3xl">
          <h2 className="text-6xl md:text-8xl font-black italic tracking-tighter leading-[0.9] text-white">
  ...
  return (
    <section className="relative h-[600px] rounded-[3rem] overflow-hidden border border-white/10 flex items-center shadow-2xl">
      ...
      <div className="relative z-20 w-full h-full flex flex-col justify-end px-10 md:px-16 pb-16">
        {ads.map((ad, idx) => (
          <div 
            key={ad.id} 
            className={`transition-all duration-700 absolute bottom-16 w-[90%] md:w-auto max-w-2xl ${idx === currentIndex ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
          >
            {(ad.title || ad.subtitle) && (
              <div className="backdrop-blur-xl bg-black/40 border border-white/10 p-8 rounded-3xl mb-6 shadow-2xl">
                {ad.subtitle && (
                ...
                {ad.title && (
                  <h2 className="text-4xl md:text-6xl font-black italic tracking-tighter leading-tight text-white drop-shadow-lg">
                    {ad.title}
                  </h2>
                )}
              </div>
      ...
      {/* Carousel Indicators */}
      {ads.length > 1 && (
        <div className="absolute bottom-8 right-10 z-30 flex gap-2">
          {ads.map((_, idx) => (
            <button 
              key={idx}
